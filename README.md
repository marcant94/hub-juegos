...
- Node.js ≥ 20 (la versión usada por el pipeline)
- pnpm (`npm install -g pnpm` o `corepack enable && corepack prepare pnpm@latest --activate`)
...
1. Job `build` en `ubuntu-latest`: pnpm 9 + Node 20, `pnpm install --frozen-lockfile` y `pnpm build`.
